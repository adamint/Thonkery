...
public class BugReportCommand extends Command {
    public BugReportCommand(String name, String help, Category category, boolean usableInDM) {
        super(name, help, category, usableInDM);
    }

    @Override
    public void execute(List<String> arguments, MessageReceivedEvent event) {
        if (arguments.size() == 0) event.getChannel().sendMessage("You need to include your bug report!").queue();
        else {
            ...
            if (channel == null) System.out.println("The bug report channel wasn't found!");
                ...
                channel.sendMessage(new EmbedBuilder()
                        .setTitle("Bug Report").setColor(Color.BLUE)
                        .addField("Username", event.getAuthor().getName() + "#" + event.getAuthor().getDiscriminator(), false)
                        .addField("Report", arguments.stream().collect(Collectors.joining(" ")), false).build()).queue();
            }
        }
    }
}
